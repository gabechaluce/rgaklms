...
if(isset($_POST['add'])){
    ...
    if(count($errors) == 0){
        ...
        try {
            ...
            $stmt_check_existing = $conn->prepare("SELECT id, quantity, price FROM purchase_master 
            WHERE product_name = ? 
            AND company_name = ? 
            AND unit = ?");
            ...
            if($existing_result->num_rows > 0){
                ...
                $purchase_id = $existing_row['id']; // NEW: Capture ID for history
                ...
                // NEW: Insert history record for update
                ...
                $purchase_id = $conn->insert_id; // NEW: Get the new ID

                // NEW: Insert history record for create
                $history_sql = "INSERT INTO purchase_history 
                              (purchase_id, inventory_selection, company_name, product_name, 
                               unit, quantity, price, party_name, purchase_type, purchase_date, action_type) 
                              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'create')";
