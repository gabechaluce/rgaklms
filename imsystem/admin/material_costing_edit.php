...
if (!isset($_GET['id']) || !is_numeric($_GET['id'])) {
    $_SESSION['error'] = ['Invalid costing ID.'];
...
if (!$qry || $qry->num_rows == 0) {
    $_SESSION['error'] = ['Material costing record not found.'];
    header("Location: material_costing.php");
    exit;
}
...
if ($_SESSION['login_type'] != 1 && $costing['created_by'] != $_SESSION['login_id']) {
    $_SESSION['error'] = ['Access denied. You don\'t have permission to edit this record.'];
    header("Location: material_costing.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['update_costing'])) {
    ...
    try {
        // Update main record
        $sql = "UPDATE material_costing SET 
                    ...
                    location = '$location',
                    quantity = $quantity,
                    remarks_finish = '$remarks_finish',
                    dimension = '$dimension',
                    materials_total = $materials_total,
                    accessories_total = $accessories_total,
                    paint_materials_total = $paint_materials_total,
                    overall_material_cost = $overall_material_cost,
                    labor_total = $labor_total,
                    jobout_total = $jobout_total,
                    grand_total = $grand_total,
                    updated_at = CURRENT_TIMESTAMP
                WHERE id = $id";
        
        if (!$conn->query($sql)) {
            throw new Exception("Error updating main record: " . $conn->error);
        }
        
        // Delete existing related records
        $conn->query("DELETE FROM material_costing_materials WHERE costing_id = $id");
        $conn->query("DELETE FROM material_costing_accessories WHERE costing_id = $id");
        $conn->query("DELETE FROM material_costing_paint_materials WHERE costing_id = $id");
        $conn->query("DELETE FROM material_costing_labor WHERE costing_id = $id");
        $conn->query("DELETE FROM material_costing_jobout WHERE costing_id = $id");
            ...
            for ($i = 0; $i < count($_POST['material_name']); $i++) {
                if (!empty($_POST['material_name'][$i])) {
                    ...
                    $sql = "INSERT INTO material_costing_materials 
                            ...
                            VALUES ($id, '$material_name', '$material_dimension', '$material_thick', 
            ...
            for ($i = 0; $i < count($_POST['accessory_name']); $i++) {
                if (!empty($_POST['accessory_name'][$i])) {
                    ...
                    $sql = "INSERT INTO material_costing_accessories 
                            ...
                            VALUES ($id, '$accessory_name', '$accessory_specification', '$accessory_thick', 
        ...
        // Insert updated paint materials
            ...
            for ($i = 0; $i < count($_POST['paint_name']); $i++) {
                if (!empty($_POST['paint_name'][$i])) {
                    ...
                    $sql = "INSERT INTO material_costing_paint_materials 
                            ...
                            VALUES ($id, '$paint_name', '$paint_specification', '$paint_thick', 
            ...
            for ($i = 0; $i < count($_POST['manpower_name']); $i++) {
                if (!empty($_POST['manpower_name'][$i])) {
                    ...
                    $sql = "INSERT INTO material_costing_labor 
                            ...
                            VALUES ($id, '$manpower_name', $person_count, $days_per_person, 
            ...
            for ($i = 0; $i < count($_POST['jobout_name']); $i++) {
                if (!empty($_POST['jobout_name'][$i])) {
                    ...
                    $sql = "INSERT INTO material_costing_jobout 
                            ...
                            VALUES ($id, '$jobout_name', '$jobout_specs', $jobout_pcs_mins, 
        ...
        $conn->commit();
        $_SESSION['success'] = 'Material costing updated successfully!';
        header("Location: material_costing.php");
        exit;
...
<head>
    ...
    <title>Edit Material Costing</title>
</head>
    ...
    <section class="content">
      ...
      <form method="post" action="">
        <div class="quotation-form">
            <div class="form-header">MATERIAL COSTING FORM - EDIT</div>
            ...
            <div class="info-section">
                <table class="form-table">
                    ...
                    <tr>
                            ...
                            <select name="project_name" id="project_name" required style="width: 100%; border: none; padding: 3px;">
                                ...
                                <?php foreach($projects as $project): ?>
                                    <option value="<?php echo htmlspecialchars($project); ?>" 
                                            <?php echo ($project == $costing['project_name']) ? 'selected' : ''; ?>>
                        ...
                        <td><input type="text" name="designer" id="designer" value="<?php echo htmlspecialchars($costing['designer']); ?>" placeholder="Enter designer name" required></td>
                    </tr>
                    <tr>
                        <td class="label-col">MODULE TITLE:</td>
                        <td><input type="text" name="module_title" id="module_title" value="<?php echo htmlspecialchars($costing['module_title']); ?>" placeholder="Enter module title"></td>
                        <td class="label-col">DATE:</td>
                        <td><input type="date" name="date" id="date" value="<?php echo $costing['date']; ?>" required></td>
                    </tr>
                    <tr>
                        <td class="label-col">LOCATION:</td>
                        <td><input type="text" name="location" id="location" value="<?php echo htmlspecialchars($costing['location']); ?>" placeholder="Enter location"></td>
                        <td class="label-col">QUANTITY:</td>
                        <td><input type="number" name="quantity" id="quantity" value="<?php echo $costing['quantity']; ?>" min="1" required></td>
                    </tr>
                    <tr>
                        <td class="label-col">REMARKS/FINISH:</td>
                        <td><input type="text" name="remarks_finish" id="remarks_finish" value="<?php echo htmlspecialchars($costing['remarks_finish']); ?>" placeholder="Enter remarks/finish"></td>
                        <td class="label-col">DIMENSION:</td>
                        <td><input type="text" name="dimension" id="dimension" value="<?php echo htmlspecialchars($costing['dimension']); ?>" placeholder="Enter dimension"></td>
                    </tr>
                </table>
            </div>
            ...
            <div class="materials-section">
                <table class="form-table">
                    ...
                    <tbody id="materials_tbody">
                        ...
                        <tr class="material-row">
                            <td><input type="text" name="material_name[]" value="<?php echo htmlspecialchars($material['material_name']); ?>" placeholder="Material name"></td>
                            <td><input type="text" name="material_dimension[]" value="<?php echo htmlspecialchars($material['dimension']); ?>" placeholder="Dimension"></td>
                            <td><input type="text" name="material_thick[]" value="<?php echo htmlspecialchars($material['thick']); ?>" placeholder="Thick"></td>
                            <td><input type="number" name="material_qty[]" value="<?php echo $material['qty']; ?>" class="number-input material-qty" min="0" step="0.01"></td>
                            <td><input type="text" name="material_uom[]" value="<?php echo htmlspecialchars($material['uom']); ?>" placeholder="UOM"></td>
                            <td><input type="number" name="material_price[]" value="<?php echo $material['unit_price']; ?>" class="number-input material-price" min="0" step="0.01"></td>
                            <td><input type="number" name="material_total[]" value="<?php echo $material['total']; ?>" class="number-input material-total" readonly></td>
                            <td><button type="button" class="btn-remove-row" onclick="removeRow(this, 'materials')">×</button></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <tr>
                        ...
                        <td class="total-row"><input type="number" name="materials_total" id="materials_total" value="<?php echo $costing['materials_total']; ?>" class="number-input" readonly></td>
                    ...
                    <tbody id="accessories_tbody">
                        ...
                        <tr class="accessory-row">
                            <td><input type="text" name="accessory_name[]" value="<?php echo htmlspecialchars($accessory['accessory_name']); ?>" placeholder="Accessory name"></td>
                            <td><input type="text" name="accessory_specification[]" value="<?php echo htmlspecialchars($accessory['specification']); ?>" placeholder="Specification"></td>
                            <td><input type="text" name="accessory_thick[]" value="<?php echo htmlspecialchars($accessory['thick']); ?>" placeholder="Thick (optional)"></td>
                            <td><input type="number" name="accessory_qty[]" value="<?php echo $accessory['qty']; ?>" class="number-input accessory-qty" min="0" step="0.01"></td>
                            <td><input type="text" name="accessory_uom[]" value="<?php echo htmlspecialchars($accessory['uom']); ?>" placeholder="UOM"></td>
                            <td><input type="number" name="accessory_price[]" value="<?php echo $accessory['unit_price']; ?>" class="number-input accessory-price" min="0" step="0.01"></td>
                            <td><input type="number" name="accessory_total[]" value="<?php echo $accessory['total']; ?>" class="number-input accessory-total" readonly></td>
                            <td><button type="button" class="btn-remove-row" onclick="removeRow(this, 'accessories')">×</button></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <tr>
                        <td colspan="6" class="total-row center-text">TOTAL:</td>
                        <td class="total-row"><input type="number" name="accessories_total" id="accessories_total" value="<?php echo $costing['accessories_total']; ?>" class="number-input" readonly></td>
                    ...
                    <tbody id="paint_materials_tbody">
                        ...
                        <tr class="paint-row">
                            <td><input type="text" name="paint_name[]" value="<?php echo htmlspecialchars($paint['paint_name']); ?>" placeholder="Paint material name"></td>
                            <td><input type="text" name="paint_specification[]" value="<?php echo htmlspecialchars($paint['specification']); ?>" placeholder="Specification"></td>
                            <td><input type="text" name="paint_thick[]" value="<?php echo htmlspecialchars($paint['thick']); ?>" placeholder="Thick (optional)"></td>
                            <td><input type="number" name="paint_qty[]" value="<?php echo $paint['qty']; ?>" class="number-input paint-qty" min="0" step="0.01"></td>
                            <td><input type="text" name="paint_uom[]" value="<?php echo htmlspecialchars($paint['uom']); ?>" placeholder="UOM"></td>
                            <td><input type="number" name="paint_price[]" value="<?php echo $paint['unit_price']; ?>" class="number-input paint-price" min="0" step="0.01"></td>
                            <td><input type="number" name="paint_total[]" value="<?php echo $paint['total']; ?>" class="number-input paint-total" readonly></td>
                            <td><button type="button" class="btn-remove-row" onclick="removeRow(this, 'paint_materials')">×</button></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <tr>
                        <td colspan="6" class="total-row center-text">TOTAL:</td>
                        <td class="total-row"><input type="number" name="paint_materials_total" id="paint_materials_total" value="<?php echo $costing['paint_materials_total']; ?>" class="number-input" readonly></td>
                    ...
                    <tr>
                        ...
                        <td colspan="2" class="overall-total-row"><input type="number" name="overall_material_cost" id="overall_material_cost" value="<?php echo $costing['overall_material_cost']; ?>" class="number-input" readonly></td>
                    </tr>
                    ...
                    <tbody id="labor_tbody">
                        ...
                        <tr class="labor-row">
                            <td><input type="text" name="manpower_name[]" value="<?php echo htmlspecialchars($lab['manpower_name']); ?>" placeholder="Manpower name"></td>
                            <td><input type="number" name="person_count[]" value="<?php echo $lab['person_count']; ?>" class="number-input person-count" min="0" step="1"></td>
                            <td><input type="number" name="days_per_person[]" value="<?php echo $lab['days_per_person']; ?>" class="number-input days-per-person" min="0" step="0.5"></td>
                            <td colspan="2"><input type="text" name="labor_uom[]" value="<?php echo htmlspecialchars($lab['uom']); ?>" placeholder="UOM"></td>
                            <td><input type="number" name="labor_price[]" value="<?php echo $lab['unit_price']; ?>" class="number-input labor-price" min="0" step="0.01"></td>
                            <td><input type="number" name="labor_total[]" value="<?php echo $lab['total']; ?>" class="number-input labor-total" readonly></td>
                            <td><button type="button" class="btn-remove-row" onclick="removeRow(this, 'labor')">×</button></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <tr>
                        <td colspan="6" class="total-row center-text">TOTAL:</td>
                        <td class="total-row"><input type="number" name="labor_total" id="labor_total" value="<?php echo $costing['labor_total']; ?>" class="number-input" readonly></td>
                    ...
                    <tbody id="jobout_tbody">
                        ...
                        <tr class="jobout-row">
                            <td><input type="text" name="jobout_name[]" value="<?php echo htmlspecialchars($job['jobout_name']); ?>" placeholder="Job out name"></td>
                            <td><input type="text" name="jobout_specs[]" value="<?php echo htmlspecialchars($job['specs']); ?>" placeholder="Specifications"></td>
                            <td><input type="number" name="jobout_pcs_mins[]" value="<?php echo $job['pcs_mins']; ?>" class="number-input jobout-pcs-mins" min="0" step="0.01"></td>
                            <td colspan="2"><input type="text" name="jobout_uom[]" value="<?php echo htmlspecialchars($job['uom']); ?>" placeholder="UOM (optional)"></td>
                            <td><input type="number" name="jobout_price[]" value="<?php echo $job['unit_price']; ?>" class="number-input jobout-price" min="0" step="0.01"></td>
                            <td><input type="number" name="jobout_total[]" value="<?php echo $job['total']; ?>" class="number-input jobout-total" readonly></td>
                            <td><button type="button" class="btn-remove-row" onclick="removeRow(this, 'jobout')">×</button></td>
                        </tr>
                        <?php endforeach; ?>
                        <?php endif; ?>
                    ...
                    <tr>
                        ...
                        <td class="total-row"><input type="number" name="jobout_total" id="jobout_total" value="<?php echo $costing['jobout_total']; ?>" class="number-input" readonly></td>
                    ...
                    <tr>
                        ...
                        <td colspan="2" class="overall-total-row"><input type="number" name="grand_total" id="grand_total" value="<?php echo $costing['grand_total']; ?>" class="number-input" readonly style="font-size: 16px; font-weight: bold;"></td>
                    </tr>
                </table>
            </div>
                ...
                <table class="form-table">
                    <tr>
                        <td colspan="8" style="text-align: center; padding: 20px;">
                            ...
                            <button type="submit" name="update_costing" class="btn btn-success">
                                <i class="fa fa-save"></i> Update Material Costing
                            </button>
                        </td>
                    </tr>
                </table>
