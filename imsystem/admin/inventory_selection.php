...
<div class="wrapper">
        ...
        <div class="col-xs-12">
          <div class="box floating-box">
            <div class="box-header with-border">
              <h3 class="box-title">Selection List</h3>
              <div class="pull-right">
                <button class="btn btn-primary btn-sm" data-toggle="modal" data-target="#addnew"><i class="fa fa-plus"></i> Add New Selection</button>
              </div>
            </div>
            <div class="box-body">
                ...
                <table id="example1" class="table table-bordered table-fit">
                  ...
                  <tbody>
                    ...
                    while ($row = $query->fetch_assoc()) {
                        // Directly use the correct column name
                        $selectionName = isset($row['inventory_selection']) ? 
                                         htmlspecialchars($row['inventory_selection']) : 
                                         'Unnamed Selection';
  ...
  <?php include 'includes/inventory_selection_modal.php'; ?>
...
function getRow(id){
  $.ajax({
    type: 'POST',
    url: 'inventory_selection_row.php',
    data: {id:id},
    dataType: 'json',
    success: function(response){
      $('.selid').val(response.id);
      
      // Check for different possible column names
      var selectionName = response.selection_name || response.name || 'Unnamed Selection';
      
      $('#edit_name').val(selectionName);
      $('#del_sel').html(selectionName);
    }
  });
}
</script>
