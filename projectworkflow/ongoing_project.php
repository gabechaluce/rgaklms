        ...
        <div class="card-body">
            <table class="table table-hover table-condensed" id="list">
                ...
                <tbody>
                    ...
                    $qry = $conn->query("SELECT * FROM project_list WHERE status = 0 && 3 ORDER BY date_created DESC, name ASC");


                    while ($row = $qry->fetch_assoc()):
                        $trans = get_html_translation_table(HTML_ENTITIES, ENT_QUOTES);
                        unset($trans["\""], $trans["<"], $trans[">"], $trans["<h2"]);
                        $desc = html_entity_decode($row['description']); // Decode HTML entities
                        $desc = str_replace("&nbsp;", " ", $desc); // Remove &nbsp;
                        $desc = trim(preg_replace('/\s+/', ' ', $desc)); // Remove extra spaces
